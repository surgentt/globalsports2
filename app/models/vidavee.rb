...
RAILS_DEFAULT_LOGGER.info("Vidavee.new_playlist:= #{key})")
  ...
  def update_asset_record(sessionid,video_asset,only={})
    ...
    puts "status of video from #{before_status} to #{video_asset.video_status} :#{Vidavee.READY}: #{(before_status != video_asset.video_status)} #{(Vidavee.READY == video_asset.video_status)} #{(video_asset.class == VideoAsset)}"
    if (before_status != video_asset.video_status)

      if (Vidavee.READY == video_asset.video_status)
        if(video_asset.class == VideoAsset)
          #video_asset.ready_at = ::DateTime.now()
          video_asset.is_ready()
        end
      elsif (video_asset.video_status == Vidavee.BLOCKED || video_asset.video_status == Vidavee.FAILED)
        # Vidavee had a problem with it, notify the admin and the owner
        fn = fullpath[File.dirname(video_asset.uploaded_file_path).length+1..-1]
...
logger.warn "Vidavee.push_video() :file_path: #{file_path}"
    # Send the post
    begin
  ...
  def push_audio(sessionid, audio_asset, file_path)

    # Build the url, yeah, with parameter on it
    url = url_for('assets/NewAssetAudio',sessionid)
    params = build_request_params('assets/NewAssetAudio',sessionid)
    url = query_url(url,params)
    
    # Send in any extra parameters the upload form requires
    #asis=file_path.downcase.end_with? '.flv'
    upload_params = [
                     Curl::PostField.content('title',"audio_id_#{audio_asset.id}"),
                     Curl::PostField.file('Asset',file_path),
                     Curl::PostField.content('tag:1:',make_tag(audio_asset))
                     ]
    # Send the post
    begin
      response = do_upload(url,*upload_params)
      ...
      response = "Vidavee.push_audio(): Could not contact Vidavee backend: #{$!}"
      logger.error "Vidavee.push_audio(): Could not contact Vidavee backend: #{$!}"
    end

    # update attributes in the asset
    if dockey_elem
      audio_asset.dockey= dockey_elem.text
      audio_asset.status= AudioAsset::STATUS_READY
      audio_asset.save!
      dockey_elem.text
    else
      audio_asset.status= AudioAsset::STATUS_ERROR
      audio_asset.internal_notes= response
      audio_asset.save!
      logger.error "Video push failed: #{response}"
      nil
    end
