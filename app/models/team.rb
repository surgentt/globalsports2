class Team < ActiveRecord::Base
  ...
  before_destroy { |team| AccessGroup.destroy_all "team_id = #{team.id}" }
  ...
  def self.schools_for(state)
    if state && state_id = State.find_by_long_name(state)
      cond = ["state_id = ? AND name IS NOT NULL", state_id]
    else
      cond = "name IS NOT NULL"
    end
...
#    }
#    member
    User.team_admin(id).collect(&:enabled).index(true)
  end
