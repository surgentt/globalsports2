class VideoClip < ActiveRecord::Base
  ...
  named_scope :for_contest,
    lambda { |contest| { :conditions=>{:created_at => contest.begin .. contest.end, 'taggings.tag_id' => contest.tag_id}, :include => :taggings } }
  ...
  def thumbnail_url
    "http://tribeca.vidavee.com/hsstv/rest/file/GetFileThumbnailMedium?dockey=#{dockey}"
  end

  def is_thumbnail_ready?
    r = Net::HTTP.get_response(URI.parse(thumbnail_url))
    r.code == "200"
  end
  ...
  def public_description
    if video_asset.league && video_asset.league.fb_share_msg
      video_asset.league.fb_share_msg
    elsif description && !description.empty?
      description
    else
      'Check out this video, and come create your own clips and highlights from the thousands of full-game videos at GlobalSports.net'
    end
  end
