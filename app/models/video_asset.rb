...
class VideoAsset < ActiveRecord::Base
  ...
  VIDEO_REPOSITORY = UPLOAD_BASE #VIDEO_BASE+"/uploaded"
  ...
  named_scope :for_user,
    lambda { |user| { :conditions => ["(team_id IN (?) || league_id IN (?)) and video_status = 'ready' and (public_video = ? || user_id = ?)",(user.league_staff? ? user.league.team_ids : [ user.team_id ]), (user.league_staff? ? [user.league_id] : []), true, user.id ] } }
  ...
  def is_ready()
    ...
    self.save!
    FileUtils.rm_f uploaded_file_path if uploaded_file_path
    if gamex_league_id
      VideoHistory.uploaded(self)
      if notify_ready
          group = access_item.first.access_group
          if group
            RosterEntry.find(:all, :conditions=>{:access_group_id => group.id}).each() { |re|
              user = re.user
              if user.nil?
                user = User.new({:email=>re.email, :firstname=>re.firstname, :lastname=>re.lastname})
                re.share
              end
              
              UserNotifier.deliver_video_ready(self, user, re) if !user.email.nil? && !user.email.empty?
            }
          end
      end
    end
  ...
  def thumbnail
    if video_status != 'ready'
      "/images/video_#{video_status}.jpg"
    else
      Vidavee.first.file_thumbnail_medium(dockey)
    end
  end
