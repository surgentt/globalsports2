...
class UserNotifier < ActionMailer::Base
  ...
  # Removed the following: '@mms.uscc.net'
  @@SMS_Gateway_Domains = Array['@teleflip.com','@message.alltel.com','@paging.acswireless.com','@txt.att.net','@bellsouth.cl','@myboostmobile.com','@sms.edgewireless.com','@messaging.sprintpcs.com','@tmomail.net','@mymetropcs.com','@messaging.nextel.com','@mobile.celloneusa.com','@qwestmp.com','@pcs.rogers.com','@msg.telus.com','@vtext.com','@vmobl.com']
  ...
  def roster_invite(options = {})
    ...
    @subject     = "Invitation from Coach #{@coach.lastname}"  #"#{AppConfig.community_name} invitation"
    @sent_on     = Time.now
    @body[:roster_entry] = @roster_entry
    @body[:coach] = @coach
    @body[:url]  = "#{APP_URL}/users/new?roster_invite_key=#{@roster_entry.reg_key}"
  end
  ...
  def billing_notice(billing)
    setup_sender_info
    content_type "text/html"
    @recipients  = "#{billing.user.email}"
    @cc          = AppConfig.support_email
    @subject     = "Your #{AppConfig.community_name} Purchase"
    @sent_on     = Time.now
  ...
  def video_ready(video, user, roster_entry)
    setup_sender_info
    content_type "text/html"
    @recipients  = "#{user.email}"
    @subject     = "Video Uploaded by Coach #{video.user.lastname}"
    @sent_on     = Time.now
  ...
  def pre_moderated_welcome(user, group)
    setup_sender_info
    content_type "text/html"

    @recipients  = "#{user.email}"
    @subject     = "#{group.class.to_s} Request Pending Moderation"
    @sent_on     = Time.now
  ...
  def moderation_request(user, group)
    setup_sender_info
    content_type "text/html"

    @recipients  = AppConfig.support_email
    @subject     = "#{group.class.to_s} #{group.name} - Moderation Requestion"
    @sent_on     = Time.now
  ...
  def new_message(sent_message,email)
    ...
    @body[:sent_message]  = sent_message
