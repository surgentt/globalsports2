...
class User < ActiveRecord::Base
  ...
  validates_exclusion_of :password, :in => ['password'], :message => "can't be blank"
  validates_exclusion_of :password_confirmation, :in => ['confirm password'], :message => "can't be blank"
  ...
  before_destroy { |me| GamexUser.destroy_all "user_id = #{me.id}" }
  before_destroy { |me| Permission.destroy_all(:blessed_type=>'User', :blessed_id=>me.id) }
...
#    end
  ...
  def scout_staff?
    role && (role.eql?(Role[:scout_staff]) || scout_admin?)
  end
  ...
  def add_default_permissions()
    ...
    Permission.staff_default_permissions.each() { |permission|
      begin
        Permission.grant(self, permission, scope)
      rescue Exception=>e
        logger.warn "error granting permission for user:#{@id}. #{e.message}"
      end
    }
  ...
  def has_access?(item)
    #asset_list = []
    Thread.current[:user_has_access] = []
    ...
    when VideoAsset
      ...
      #return pass
      return has_access_to_asset?(item)
    ...
    when VideoClip
      ...
      #pass = has_access_to_clip?(item) || (!needs_access_to_clip?(item) && has_access_to_asset?(asset))
      #return pass
      return has_access_to_clip?(item)
    ...
    when VideoReel
      ...
      Vidavee.first.get_clip_dockeys_for_reel(item.dockey).each() { |dockey|
        ...
        clip_list.each() { |clip|
          Thread.current[:user_has_access] << "contains clip #{clip.id}"
          asset = clip.video_asset
          #pass = team_staff?(asset.team) || has_access_to_clip?(item) || has_access_to_asset?(asset)
          pass = has_access_to_clip?(item) || has_access_to_asset?(asset)
  ...
  def has_access_to_asset?(item)
    ...
    #return true if admin? || team_staff?(item.team)
    if admin?
      Thread.current[:user_has_access] << "is admin"
      return true
    end

    if team_staff?(item.team)
      Thread.current[:user_has_access] << "is team_staff"
      return true
    end
    ...
    if gamex_asset_id
      Thread.current[:user_has_access] << "gamex controlled"
      ...
      if 0 < GamexUser.count(:conditions => { :user_id => id, :league_id => gamex_asset_id } )
        ...
        Thread.current[:user_has_access] << "is gamex coach"
        
        #return gamex_league ? gamex_league.release?(item) : true
        if gamex_league
          released = gamex_league.release?(item)
          Thread.current[:user_has_access] << "gamex released: #{released}"
          return released
        else
          Thread.current[:user_has_access] << "no gamex league release date"
          return true
        end
      else
        logger.info("has_access? isn't coach but this is a gamex video, passing to access group check")
        Thread.current[:user_has_access] << "not a gamex coach"

        #are we in an access group associated with the gamex league
        gamex_access_groups = GamexUser.for_league(gamex_asset_id).collect(&:access_group_id)
        user_access_groups = AccessUser.for_user(self).collect(&:access_group_id)
        pass_groups = gamex_access_groups & user_access_groups
        
        if !pass_groups.empty?
          Thread.current[:user_has_access] << "in access_group(s) #{pass_groups.join('.')} associated with gamex league"
          return true
        end
...
#        access_pass = false
    ...
    if !enabled?
      Thread.current[:user_has_access] << "ppv user"
      if !can_ppv?(item)
        Thread.current[:user_has_access] << "not purchased"
        return false
      end
    end
    ...
    if access_items.any?
      Thread.current[:user_has_access] << "access group controlled"
      access_pass = false
      logger.info("has_access? #{access_pass}")
      access_items.each{ |access_item|
        ...
        Thread.current[:user_has_access] << "member of access group #{access_item.access_group_id}: #{access_pass}"
      }
    end
    logger.info("has_access? #{access_pass}")
  ...
  def has_access_to_clip?(item)
    if needs_access_to_clip?(item)
      (AccessItem.for_item(item) || []).each{ |access_item|
        pass = (access_item.access_group.enabled && access_item.access_group.allow?(self) )
        Thread.current[:user_has_access] << "member of access group #{access_item.access_group_id}: #{pass}"
        return true if pass
      }
      return false
    end
...
#
  ...
  def enable
      self.enabled = true
      self.activated_at = Time.now if self.activated_at.nil?
