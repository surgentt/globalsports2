class VideoClipsController < BaseController
  ...
  skip_before_filter :gs_login_required, :only => [ :show, :panel ]
  
  #before_filter :gs_user_required, :only => [ :show ]
  ...
  def panel
    ...
    @video_clip = VideoClip.find(:first, :conditions=>{ :dockey => dockey }) rescue nil
    #
    #if @video_clip
    render :layout => false
    #else
    #  render :action=>'panel_loading', :layout => false
    #end
