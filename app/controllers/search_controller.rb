class SearchController < BaseController
  ...
  def my_videos
    ...
    @video_assets = VideoAsset.for_user(@user).all(:limit => 10, :order => 'created_at DESC')
    @video_assets.delete_if() { |v|
      !current_user.has_access?(v)
    }
    @video_clips = VideoClip.for_user(@user).all(:limit => 10, :order => 'created_at DESC')
    @video_clips.delete_if() { |v|
      !current_user.has_access?(v)
    }
    @video_reels = VideoReel.for_user(@user).all(:limit => 10, :order => 'created_at DESC')
    @video_reels.delete_if() { |v|
      !current_user.has_access?(v)
    }
    @video_users = VideoUser.for_user(@user).all(:limit => 10, :order => 'created_at DESC')
    protect_private_videos(@video_assets)
  ...
  def video_metadata_xml(video, deep=true)
    ...
    when VideoAsset
      ...
      xstr = video.to_xml(xml_options) do |xml|
        ...
        if audio_timeline = AudioTimeline.for_dockey(video.dockey) #find(:first, :conditions=>{:video_asset_id=>video.id} )
          audio_timeline.to_xml(xml)
        end
      end
    when VideoUser
      ...
      xstr = video.to_xml(xml_options) do |xml|
        ...
        if audio_timeline = AudioTimeline.for_dockey(video.dockey)
          audio_timeline.to_xml(xml)
        end
      end
    when VideoClip
