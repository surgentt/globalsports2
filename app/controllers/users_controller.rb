class UsersController < BaseController
  ...
  before_filter :admin_required, :only => [:index, :logins, :assume, :destroy, :featured, :toggle_featured, :toggle_moderator, :disable, :registrations, :edit_promotion, :update_promotion, :test ]
  before_filter :find_user, :only => [:edit, :edit_pro_details, :show, :update, :destroy, :statistics, :disable, :test ]
  ...
  sortable_attributes :id, :firstname, :lastname, :team_id, :league_id, :promotion_id, 'memberships.billing_method', :email, :role_id, 'teams.name', 'last', 'count', 'u.firstname', 'u.lastname', 'user_id', :enabled
  ...
  def test
    ...
    target = Kernel.const_get(test_type).find(test_id) rescue nil
    
    @user_test = {}
    @user_test[:target]     = target
    @user_test[:has_access] = @user.has_access?(target) if target
    @user_test[:reasons]    = Thread.current[:user_has_access]
    show()
  end
  ...
  def new
    ...
    if cookies[:roster_invite_key] || params[:roster_invite_key]
      @roster_entry = RosterEntry.for_invite_key(params[:roster_invite_key] || cookies[:roster_invite_key]).first
      if @roster_entry
        ...
        @team      = @roster_entry.access_group.team
        @user.team = @team
        @user.league = @team.league if (@team.league)
...
#      :partner => Active_Merchant_payflow_gateway_partner)
  ...
  def create
    ...
    when Role[:team].id 
      ...
      begin
        @team = Team.find(params[:team][:id]) rescue Team.find(1)
        @user.team = @team
        if (@team.league)
          @user.league = @team.league
        end
    ...
    # common stuff for all registration types
    ...
    unless params[:promo_code].blank?
      ...
      if @promotion == nil || !@promotion.enabled? # ||
           # (@promotion.subscription_plan_id != nil && @role.plan != nil && @role.plan.id != @promotion.subscription_plan_id)
        ...
        #@promotion.errors.add('', "Sorry, the promotion code you entered is invalid: #{params[:promo_code]}.") if @promotion
        #raise Exception.new, "Invalid promotion code"
        #flash[:notice] = "Invalid promotion code"
      else
    ...
    when Role[:team].id
      ...
      UserNotifier.deliver_moderation_request(@user, @team)
      UserNotifier.deliver_pre_moderated_welcome(@user, @team)

      redirect_to :controller => 'users', :action => 'moderated'
    ...
    when Role[:league].id
      ...
      UserNotifier.deliver_moderation_request(@user, @league)
      UserNotifier.deliver_pre_moderated_welcome(@user, @team)
...
#      notify_new_user()
      ...
      flash[:notice] = "Welcome to Global Sports!"
      ...
      #session[:return_to] ? redirect_to(session[:return_to]) : redirect_to(signup_completed_user_path(@user))
      #session[:return_to] = nil

      params[:session_return_to] ? redirect_to(params[:session_return_to]) : redirect_to(signup_completed_user_path(@user))



    end

    @user.add_default_permissions()
  ...
  def store_roster_invite()
    @roster_entry = RosterEntry.find_by_email(@user.email) || (cookies[:roster_invite_key] && RosterEntry.for_invite_key(cookies[:roster_invite_key]).first)
    #if @user.lastname == @roster_entry.lastname

    if @roster_entry
      access = AccessUser.new()
      access.user = @user
      access.access_group_id = @roster_entry.access_group_id
      access.save
...
logger.info("AAAA #{!@user.team_id} ||  #{@user.team_id == 1} => #{h @user.team_id.inspect} #{@user.team_id.class} ; #{h @user.team.id.inspect} #{@user.team.id.class}")
      if !@user.team_id || @user.team_id == 1
        @user.team = @roster_entry.access_group.team
        @user.league = @user.team.league
logger.info("AAAA #{!@user.team_id} ||  #{@user.team_id == 1} => #{h @user.team_id.inspect} #{@user.team_id.class} ; #{h @user.team.id.inspect} #{@user.team.id.class}")
        @user.save
      end
logger.info("AAAA #{!@user.team_id} ||  #{@user.team_id == 1} => #{h @user.team_id.inspect} #{@user.team_id.class} ; #{h @user.team.id.inspect} #{@user.team.id.class}")
  ...
  def setup_access_groups()

    logger.info "REGWATCH * adding user to promotion access group?"
    #adding user to promotion access group
    begin
      access_group = @promotion ? @promotion.access_group : nil

      # very special case
      if @promotion && @promotion.promo_code.downcase == "chsfl"
        logger.info "REGWATCH * CHSFL * looking for Football"
        access_group = AccessGroup.find(:first, :conditions=>{:name=>'Football', :team_id=>@user.team_id})
      end
  ...
  # MOVED TO User.rb
  def xadd_default_permissions(user)
  ...
  def update
    ...
    if @user.save!
      ...
      @user.enable() if @user.enabled and !was_enabled
  ...
  def index
    @users = User.paginate :all, :order=>sort_order, :page => params[:page]
  end
