class TeamsController < BaseController
  ...
  skip_before_filter :verify_authenticity_token, :only => [:auto_complete_for_team_name, :auto_complete_for_team_league_name, :school_list]
  before_filter :admin_required, :only => [:index, :new, :create, :destroy]
  #before_filter :admin_for_league_or_team, :only => [:edit, :update]
  skip_before_filter :gs_login_required, :only => [:show_public, :photo_gallery, :find_my_school, :school_list]
  ...
  def find_my_school
    @states = Team.states.map {|s| State.find s.id}
  end

  def school_list
    @schools = Team.schools_for(params[:state])
