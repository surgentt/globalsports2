...
#    file = @video_user.uploaded_file_path #params[:uploaded_file_path] # the tmp file
#    filename = @video_user.uploaded_file_path.original_filename #title.gsub(/[^a-zA-Z0-9]/,'')
#    fpath = VideoUser.move_upload_to_repository(file,filename)
#    @video_user.uploaded_file_path = fpath
    ...
    if @video_user.save
      #render :action => :upload_success, :layout=>false
      render :json => {
        :asset_type => @video_user.class().name(),
        :asset_id => @video_user.id,
        :link => video_user_path(@video_user)
      }
    else
      ...
      @video_user.errors.each() {|attr, msg|
        err << { :attr => attr, :msg => msg }
      }
